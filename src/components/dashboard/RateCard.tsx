...
import { TrendingUp, TrendingDown, Coins, DollarSign, Bitcoin } from 'lucide-react';
...
export function RateCard({ asset }: RateCardProps) {
    ...
    <div className="group relative flex flex-col justify-between rounded-[24px] border border-slate-200 bg-white p-5 shadow-[0_12px_24px_rgba(148,163,184,0.08)] hover:shadow-[0_18px_40px_rgba(148,163,184,0.15)] transition-all duration-300 hover:-translate-y-1">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="relative w-10 h-10 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center overflow-hidden shrink-0">
             {flagUrl ? (
                <img src={flagUrl} alt={asset.symbol} className="w-full h-full object-cover opacity-90" />
             ) : (
                <span className="text-xs font-bold text-slate-400">{asset.symbol.slice(0, 2)}</span>
             )}
          </div>
          <div className="flex flex-col">
            <span className="text-sm font-bold text-slate-800 leading-tight group-hover:text-blue-700 transition-colors">
              {asset.nameFa}
            </span>
        ...
        <div className={cn("h-8 w-8 rounded-full flex items-center justify-center border", getCategoryColor())}>
          {getCategoryIcon()}
        </div>
      </div>
      ...
      <div className="flex items-end justify-between">
        <div className="flex flex-col">
          ...
          <span className="text-[11px] text-slate-400 font-medium mt-1">تومان</span>
        </div>
      ...
      <div className={cn(
        "absolute -right-6 -top-6 w-24 h-24 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-2xl pointer-events-none",
